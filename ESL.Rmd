...
3.  
Our predicttion point(target point)'s expected squared distance is:

<center>
E(${\left\|x_0 \right\|}^2$) = $\sum^p_i E(x_{0i}^2)$=$\sum^p_i (Var(x_{0i}^2) + E(x_{0i})^2)$ = $\sum^p_i Var(x_{0i}^2)$ = $\sum^p_i 1$ = p.
</center>
...
*The conditional variance* $Var(y_0|x_0)$ = $Var(x_o^T\beta + \epsilon|x_0)$ = $Var(\epsilon)$ = $\sigma^2$ *and*
$E_{\tau}\hat{y_0} = E(y_0|x_0) = x_0^T\beta$, so $Bias(\hat{y_0})^2 = 0$
...
Since $E_{x_0}(x_0^Tx_0)$ = $E_{x_0}(\sum{(X_{0i})^2})$ = $E_{x_0}(\sum{(X_{0i}-0)^2})$ = $E_{x_0}(\sum{(X_{0i}-E(X_0))^2})$ = Cov(X)
...
RSS($\theta$) = $\sum_{i=1}^{n=N} (y_i - \hat y_i)^2$

= $\sum_{i=1}^m \sum_{j=1}^{m_i} [(y_{ij} - \bar y_{i.})^2 + 2(y_{ij} - \bar y_{i.})(y_{i.} - \hat y_i) + (y_{i.} - \hat y_i)^2]$

First consider the second $\sum_{i=1}^m \sum_{j=1}^{m_i}2(y_{ij} - \bar y_{i.})(y_{i.} - \hat y_i)$

= $2\sum_{i=1}^m(y_{i.} - \hat y_i)\sum_{j=1}^{m_i}(y_{ij} - \bar y_{i.})$

=  0

The third term = $\sum_{i=1}^m \sum_{j=1}^{m_i} (y_{i.} - \hat y_i)^2$
...
where $x_{(k)}$ is the training observation ranked kth in distance from $x_0$ and $x_i \in N_k(x)$ 

 
$\hat f(x_0) =  Ave(y_i|x_i \in N_k(x))$
= $\sum_{i=1}^k \frac{1}{k}I(\left\|x_i-x_0\right\| \le \left\|x_{(k)}-x_0\right\|)$

Hence $\ell(x_0;\mathcal X)$  = $I(\left\|x_i-x_0\right\| \le \left\|x_{(k)}-x_0\right\|)$
...
The third term is $2E_{\mathcal Y|{\mathcal X}}(E_{\mathcal Y|\mathcal X}{\hat f(x_0)} - \hat f{(x_0)})E_{\mathcal Y|\mathcal X}(f(x_0) − E_{\mathcal Y|\mathcal X}{\hat f(x_0)}) = 0$ 
...
Hence $E_{\mathcal Y,\mathcal X} (f(x_0) − \hat f{(x_0)} )^2$ = 
